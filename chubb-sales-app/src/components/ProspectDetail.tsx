...
import type { Prospect, Action } from '../types/types';
import { STATUS_COLORS, STATUS_LABELS, SECTOR_LABELS } from '../types/types';
import { addAction, generateId } from '../utils/storage';
    ...
    return (
        <div className="h-full flex flex-col bg-gray-50">
            ...
            <div className="flex-1 overflow-auto p-6">
                {activeTab === 'info' && (
                    <InfoTab prospect={prospect} onUpdate={onUpdate} />
                )}
                {activeTab === 'soncas' && (
                    <SoncasTab prospect={prospect} onUpdate={onUpdate} />
                )}
                {activeTab === 'actions' && (
                    <ActionsTab prospect={prospect} actions={actions} onAddAction={onAddAction} />
                )}
                {activeTab === 'notes' && (
                    <div className="max-w-2xl">
                        ...
                        <div className="bg-white rounded-xl p-4 mb-4 shadow-sm">
                            ...
                            <button
                                ...
                                className="mt-2 px-4 py-2 bg-[#c8102e] text-white rounded-lg hover:bg-[#a00d24]"
                            >
                                Ajouter
                            </button>
                        </div>
            ...
            <div className="space-y-3">
                ...
                <div>
                    ...
                    <input
                        ...
                        onChange={(e) => onUpdate({ potentialValue: Number(e.target.value) })}
                        className="w-full mt-1 px-3 py-2 border border-gray-200 rounded-lg"
                    />
                </div>
                <div>
                ...
                <Field label="Date conclusion estimée" value={prospect.expectedDate || 'Non définie'} />
            </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm">
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-3">
                    <input
                        type="checkbox"
                        checked={prospect.hasProjects}
                        onChange={(e) => onUpdate({ hasProjects: e.target.checked })}
                        className="w-4 h-4"
                    />
                    <span>Le client a des projets en cours</span>
                </div>
                {prospect.hasProjects && (
                    <>
                        <Field label="Description du projet" value={prospect.projectDescription || '-'} />
                        <Field label="Échéance" value={prospect.projectDeadline || '-'} />
                    </>
                )}
            </div>
        </div>
...
// Tab SONCAS
const SoncasTab: React.FC<{
    prospect: Prospect;
    onUpdate: (updates: Partial<Prospect>) => void;
}> = ({ prospect, onUpdate }) => {
    const soncasItems = [
        { key: 'securite', label: 'Sécurité', icon: '🛡️', desc: 'Besoin de protection, conformité' },
        { key: 'orgueil', label: 'Orgueil', icon: '👑', desc: 'Image, prestige, reconnaissance' },
        { key: 'nouveaute', label: 'Nouveauté', icon: '✨', desc: 'Innovation, modernisation' },
        { key: 'confort', label: 'Confort', icon: '🛋️', desc: 'Facilité, simplicité, maintenance' },
        { key: 'argent', label: 'Argent', icon: '💰', desc: 'Budget, ROI, économies' },
        { key: 'sympathie', label: 'Sympathie', icon: '🤝', desc: 'Relation, confiance, affinité' },
    ] as const;

    const handleChange = (key: keyof typeof prospect.soncas, value: number) => {
        onUpdate({
            soncas: { ...prospect.soncas, [key]: value }
        });
    };
    ...
    return (
        <div className="max-w-2xl">
            ...
            <p className="text-gray-500 text-sm mb-6">
                Évaluez les motivations d'achat du prospect sur chaque dimension
            </p>
                ...
                {soncasItems.map(({ key, label, icon, desc }) => (
                    <div key={key} className="bg-white rounded-xl p-4 shadow-sm">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <div className="ml-auto text-xl font-bold text-[#c8102e]">
                                {prospect.soncas[key]}/10
                            </div>
                        </div>
                        <input
                            type="range"
...
const ActionsTab: React.FC<{
    ...
    actions: Action[];
    onAddAction: (action: Action) => void;
}> = ({ prospect, actions, onAddAction }) => {
    const [showForm, setShowForm] = useState(false);
    const [newAction, setNewAction] = useState({
        type: 'appel' as Action['type'],
        title: '',
        description: '',
        ...
        <div className="max-w-2xl">
            <div className="flex items-center justify-between mb-4">
                ...
                <button
                    ...
                    className="px-4 py-2 bg-[#c8102e] text-white rounded-lg hover:bg-[#a00d24]"
                >
                    + Action
                </button>
            </div>
            ...
            {showForm && (
                <div className="bg-white rounded-xl p-4 mb-4 shadow-sm">
                    ...
                    <div className="flex gap-2 mt-3">
                        <button onClick={handleSubmit} className="px-4 py-2 bg-[#c8102e] text-white rounded-lg">
                            Créer
                        </button>
                        <button onClick={() => setShowForm(false)} className="px-4 py-2 bg-gray-200 rounded-lg">
                            Annuler
                        </button>
                    </div>
                </div>
