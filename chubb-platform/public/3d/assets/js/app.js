...
/**
 * Initialize device mode from localStorage or auto-detect
 */
function initDeviceMode() {
    // Check localStorage first
    const saved = localStorage.getItem('chubb_detection_device_mode');
    ...
    setDeviceMode(isTouch ? 'tablet' : 'pc', false);
}

/**
 * Set device mode (PC souris+clavier or iPad stylet+tactile)
 * @param {string} mode - 'pc' or 'tablet'
...
function setDeviceMode(mode, save = true) {
    ...
    if (btnPC && btnTablet) {
        ...
        btnTablet.classList.toggle('tablet-active', mode === 'tablet');
    }

    // Adapt OrbitControls for touch
    if (controls) {
        if (mode === 'tablet') {
    ...
    // Save preference
    if (save) {
        localStorage.setItem('chubb_detection_device_mode', mode);
    }
    ...
    if (toast && txt) {
        txt.innerText = mode === 'tablet' ? 'Mode iPad activé' : 'Mode PC activé';
...
// ============================================
// MATERIAL SYSTEM & REALISTIC RENDERING
// ============================================
...
const materialLibrary = {
    walls: {
        plaster: { color: 0xf5f5f5, roughness: 0.95, metalness: 0.0 },
        concrete: { color: 0x888888, roughness: 0.85, metalness: 0.0 },
        brick: { color: 0xa0522d, roughness: 0.9, metalness: 0.0 },
        glass: { color: 0xffffff, roughness: 0.0, metalness: 0.0, transparent: true, opacity: 0.2 },
        metal: { color: 0xaaaaaa, roughness: 0.3, metalness: 0.8 }
    },
    floor: {
        tile: { color: 0xe0e0e0, roughness: 0.3, metalness: 0.1 },
        wood: { color: 0x8b4513, roughness: 0.6, metalness: 0.0 },
        carpet: { color: 0x4a4a5a, roughness: 1.0, metalness: 0.0 },
        concrete: { color: 0x6b6b6b, roughness: 0.7, metalness: 0.0 },
        epoxy: { color: 0x708090, roughness: 0.2, metalness: 0.1 }
    },
    ceiling: {
        plaster: { color: 0xffffff, roughness: 0.95, metalness: 0.0 },
        metal: { color: 0xcccccc, roughness: 0.4, metalness: 0.6 },
        wood: { color: 0x654321, roughness: 0.7, metalness: 0.0 }
    },
    doors: {
        wood: { color: 0x8b4513, roughness: 0.6, metalness: 0.0 },
        metal: { color: 0x808080, roughness: 0.4, metalness: 0.7 },
        glass: { color: 0xadd8e6, roughness: 0.0, metalness: 0.0, transparent: true, opacity: 0.4 },
        fire: { color: 0x505050, roughness: 0.5, metalness: 0.3 }
    },
    detectors: {
        white: { color: 0xffffff, roughness: 0.6, metalness: 0.0 },
        gray: { color: 0x808080, roughness: 0.5, metalness: 0.0 },
        chrome: { color: 0xcccccc, roughness: 0.1, metalness: 0.9 }
    },
    vesda: {
        black: { color: 0x1a1a1a, roughness: 0.4, metalness: 0.6 },
        gray: { color: 0x505050, roughness: 0.4, metalness: 0.5 },
        red: { color: 0xdc2626, roughness: 0.5, metalness: 0.4 }
    },
    vesdapipe: {
        red: { color: 0xdc2626, roughness: 0.6, metalness: 0.0 },
        gray: { color: 0x808080, roughness: 0.6, metalness: 0.0 },
        copper: { color: 0xb87333, roughness: 0.3, metalness: 0.8 }
    },
    cylinders: {
        green: { color: 0x22c55e, roughness: 0.4, metalness: 0.5 },
        steel: { color: 0xc0c0c0, roughness: 0.2, metalness: 0.8 },
        black: { color: 0x1a1a1a, roughness: 0.6, metalness: 0.3 }
    },
    pipes: {
        black: { color: 0x1a1a1a, roughness: 0.5, metalness: 0.6 },
        galva: { color: 0x9a9a9a, roughness: 0.4, metalness: 0.7 },
        copper: { color: 0xb87333, roughness: 0.3, metalness: 0.85 },
        stainless: { color: 0xd0d0d0, roughness: 0.2, metalness: 0.9 }
    },
    nozzles: {
        chrome: { color: 0xcccccc, roughness: 0.1, metalness: 0.95 },
        brass: { color: 0xb5a642, roughness: 0.3, metalness: 0.8 },
        stainless: { color: 0xe0e0e0, roughness: 0.15, metalness: 0.9 }
    },
    racks: {
        black: { color: 0x1a1a1a, roughness: 0.5, metalness: 0.4 },
        gray: { color: 0x505050, roughness: 0.5, metalness: 0.4 },
        white: { color: 0xf0f0f0, roughness: 0.6, metalness: 0.2 }
    },
    beds: {
        wood: { color: 0x5d4037, roughness: 0.7, metalness: 0.0 },
        metal: { color: 0xf0f0f0, roughness: 0.4, metalness: 0.5 },
        chrome: { color: 0xcccccc, roughness: 0.1, metalness: 0.9 }
    }
};
...
function toggleRenderMode() {
    ...
    if (isRenderMode) {
        btn.innerHTML = '<i class="fas fa-eye-slash"></i> Désactiver Rendu';
        ...
        btn.innerHTML = '<i class="fas fa-eye"></i> Activer Rendu Réaliste';
        btn.classList.remove('btn-active');
        resetToSimpleMaterials();
        resetLighting();
...
function applyRealisticMaterials() {
    const selections = getMaterialSelections();

    // Apply to walls
    applyMaterialToGroup(buildingGroup, 'wall', materialLibrary.walls[selections.walls]);

    // Apply to floors
    applyMaterialToGroup(buildingGroup, 'floor', materialLibrary.floor[selections.floor]);
    applyMaterialToGroup(furnitureGroup, 'floor', materialLibrary.floor[selections.floor]);

    // Apply to doors
    applyMaterialToGroup(manualGroup, 'door', materialLibrary.doors[selections.doors]);

    // Apply to detectors
    applyMaterialToGroup(manualGroup, 'detector', materialLibrary.detectors[selections.detectors]);

    // Apply to VESDA
    applyMaterialToGroup(manualGroup, 'vesda', materialLibrary.vesda[selections.vesda]);
    applyMaterialToGroup(detectionGroup, 'vesdaPipe', materialLibrary.vesdapipe[selections.vesdapipe]);

    // Apply to extinction system
    applyMaterialToGroup(extinctionGroup, 'cylinder', materialLibrary.cylinders[selections.cylinders]);
    applyMaterialToGroup(extinctionGroup, 'pipe', materialLibrary.pipes[selections.pipes]);
    applyMaterialToGroup(extinctionGroup, 'nozzle', materialLibrary.nozzles[selections.nozzles]);

    // Apply to furniture
    applyMaterialToGroup(furnitureGroup, 'rack', materialLibrary.racks[selections.racks]);
    applyMaterialToGroup(furnitureGroup, 'bed', materialLibrary.beds[selections.beds]);

    // Enable shadows on all meshes
    enableShadowsOnGroup(scene);
}

function getMaterialSelections() {
    return {
        walls: document.getElementById('mat-walls')?.value || 'plaster',
        floor: document.getElementById('mat-floor')?.value || 'tile',
        ceiling: document.getElementById('mat-ceiling')?.value || 'plaster',
        doors: document.getElementById('mat-doors')?.value || 'wood',
        detectors: document.getElementById('mat-detectors')?.value || 'white',
        vesda: document.getElementById('mat-vesda')?.value || 'black',
        vesdapipe: document.getElementById('mat-vesdapipe')?.value || 'red',
        cylinders: document.getElementById('mat-cylinders')?.value || 'green',
        pipes: document.getElementById('mat-pipes')?.value || 'black',
        nozzles: document.getElementById('mat-nozzles')?.value || 'chrome',
        racks: document.getElementById('mat-racks')?.value || 'black',
        beds: document.getElementById('mat-beds')?.value || 'wood'
    };
}

function applyMaterialToGroup(group, type, matProps) {
    if (!matProps) return;

    group.traverse(child => {
        if (child.isMesh && child.material && child.material.visible !== false) {
            const materialName = child.material.name || '';
            const shouldApply = matchesMaterialType(child, type);
...
function matchesMaterialType(mesh, type) {
    const mat = mesh.material;
    if (!mat) return false;

    // Match by color or position in hierarchy
    const color = mat.color ? mat.color.getHex() : 0;

    switch (type) {
        case 'wall':
            return color === 0xf5f5f5 || mat === materials.wall;
        case 'floor':
            return color === 0xd2b48c || mat === materials.floorRoom || mat === materials.floorBath;
        case 'door':
            return color === 0x8d6e63 || mat === materials.door;
        case 'detector':
            return mat === materials.detector;
        case 'vesda':
            return mat === materials.vesdaBox;
        case 'vesdaPipe':
            return mat === materials.vesdaPipe;
        case 'cylinder':
            return mat === materials.ig55Cylinder || mat === materials.ig55Shoulder;
        case 'pipe':
            return mat === materials.pipeBlack;
        case 'nozzle':
            return mat === materials.nozzleChrome;
        case 'rack':
            return color === 0x1a1a1a && mesh.geometry?.type === 'BoxGeometry';
        case 'bed':
            return mat === materials.wood || mat === materials.bedSheet;
        default:
            return false;
    }
}

function enableShadowsOnGroup(group) {
    group.traverse(child => {
        if (child.isMesh) {
...
function enhanceLighting() {
    // Store and remove existing lights
    originalLights = scene.children.filter(c => c.isLight);
    originalLights.forEach(l => scene.remove(l));

    // Ambient light (soft fill)
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.3);
    ambientLight.name = 'renderAmbient';
    ...
    sunLight.name = 'renderSun';
    scene.add(sunLight);

    // Fill light (softer from opposite side)
    const fillLight = new THREE.DirectionalLight(0x87ceeb, 0.4);
    fillLight.position.set(-10, 15, -10);
    fillLight.name = 'renderFill';
    scene.add(fillLight);

    // Rim light for depth
    const rimLight = new THREE.DirectionalLight(0xffffff, 0.3);
    rimLight.position.set(0, 5, -20);
    rimLight.name = 'renderRim';
    scene.add(rimLight);

    // Update based on UI settings
    updateLighting();
}

function resetLighting() {
    // Remove render lights
    const renderLights = scene.children.filter(c => c.isLight && c.name.startsWith('render'));
    renderLights.forEach(l => scene.remove(l));

    // Restore original lights
    if (originalLights.length > 0) {
        originalLights.forEach(l => scene.add(l));
    } else {
        // Default lights
        const ambLight = new THREE.AmbientLight(0xffffff, 0.6);
        scene.add(ambLight);
...
function resetToSimpleMaterials() {
    // Rebuild scene to reset materials
    updateAll();
}
...
function updateLighting() {
    ...
    const temp = document.getElementById('light-temp')?.value || 'neutral';

    // Update intensity display
    const valueSpan = document.getElementById('light-value');
    if (valueSpan) valueSpan.textContent = intensity.toFixed(1);

    // Color temperature
    let lightColor;
    switch (temp) {
        case 'warm': lightColor = 0xffd4a3; break;
        case 'cool': lightColor = 0xa3d4ff; break;
        default: lightColor = 0xffffff;
    }

    // Apply to render lights
    const sunLight = scene.getObjectByName('renderSun');
    if (sunLight) {
        sunLight.intensity = intensity;
        sunLight.color.setHex(lightColor);
    }

    const ambientLight = scene.getObjectByName('renderAmbient');
    if (ambientLight) {
