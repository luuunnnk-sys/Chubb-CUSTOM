...
import { getProspects, getActions, getEvents, saveProspects, addProspect, updateProspect, deleteProspect, calculateScore, generateId } from './storage';
...
function SalesApp({ deviceMode = 'pc' }: SalesAppProps) {
    ...
    return (
        <div className={`flex h-full bg-gray-50 ${isTablet ? 'text-sm' : ''}`}>
            {/* Sidebar */}
            <Sidebar
                currentView={view}
                onChangeView={(v) => {
